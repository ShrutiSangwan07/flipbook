...
// --- BG Music: play automatically, unmuted on mobile ---
const bgMusic = document.getElementById('bgMusic');

// Ensures music will start on first user interaction if auto-play fails (mobile-safe)
function setupMobileMusicPlayback() {
  if (!bgMusic) return;
  bgMusic.volume = 0.15;
  bgMusic.muted = false;
  ...
  function resumeOnGesture() {
    bgMusic.muted = false;
    bgMusic.volume = 0.15;
    // Try play again in a gesture event
    bgMusic.play().catch(()=>{});
  ...
  // Listen for first eligible event
  window.addEventListener("click", resumeOnGesture, { once: true });
  window.addEventListener("keydown", resumeOnGesture, { once: true });
  window.addEventListener("touchstart", resumeOnGesture, { once: true });
}

// Alters to guarantee the play and fallback interaction setup always fire
function tryPlayMusic() {
  if (!bgMusic) return;
  bgMusic.volume = 0.15;
  bgMusic.muted = false;
  // Always try, even repeatedly.
  const playPromise = bgMusic.play();
  if (playPromise && typeof playPromise.then === "function") {
    playPromise.catch(() => {
      // Mobile will land here; set up gesture fallback
      setupMobileMusicPlayback();
    });
  } else {
    // If play returns undefined or not a promise, still attempt fallback
    setupMobileMusicPlayback();
  }
}

// Play immediately if possible, and always set up Fallback
document.addEventListener("DOMContentLoaded", tryPlayMusic);
window.addEventListener("load", tryPlayMusic);

// Always also setup fallback in case auto-play is not even attempted on DOM ready (older iOS/Safari)
setupMobileMusicPlayback();
